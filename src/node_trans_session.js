...
class NodeTransSession extends EventEmitter {
    ...
    run() {
        ...
        let ouPath = `${this.conf.mediaroot}/${this.conf.streamApp}`;
        let mapStr = '';
        ...
        if (this.conf.mp4) {
            ...
            //   let mp4FileName = dateFormat('yyyy-mm-dd-HH-MM-ss') + '666.mp4';
            let mp4FileName = this.conf.streamName + '.mp4';
            let mapMp4 = `${this.conf.mp4Flags}${ouPath}/${mp4FileName}`;
            mapStr += mapMp4;
            Logger.log(
                '[Transmuxing MP4] ' + this.conf.streamPath + ' to ' + ouPath + '/' + mp4FileName
            );
        }
        if (this.conf.hls) {
            ...
            mapStr += mapHls;
            Logger.log(
                '[Transmuxing HLS] ' + this.conf.streamPath + ' to ' + ouPath + '/' + hlsFileName
            );
        }
        if (this.conf.dash) {
            ...
            mapStr += mapDash;
            Logger.log(
                '[Transmuxing DASH] ' + this.conf.streamPath + ' to ' + ouPath + '/' + dashFileName
            );
        }
        mkdirp.sync(ouPath);
        let argv = ['-y', '-i', inPath];
